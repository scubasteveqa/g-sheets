...
server <- function(input, output, session) {
  ...
  observeEvent(input$create_sheet, {
    ...
    new_sheet_id <- NULL
    tryCatch({
      cat("Attempting to create sheet using drive_create:", input$new_sheet_name, "\n")

      # Try using googledrive to create a spreadsheet file
      # This might bypass whatever is blocking gs4_create
      ...
      cat("Sheet created via drive_create! ID:", new_sheet_id, "\n")
      
      # Now add initial column headers using googlesheets4
      tryCatch({
        ...
        sheet_write(initial_data, ss = new_sheet_id, sheet = 1)
